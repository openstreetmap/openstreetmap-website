  ...
  class ChangesetsControllerTest < ActionController::TestCase
    ...
    def test_routes
      all_api_versions.each do |api_version|
        assert_routing(
          { :path => "/api/#{api_version}/changeset/create", :method => :put },
          { :controller => "api/changesets", :action => "create", :api_version => api_version }
        )
        assert_routing(
          { :path => "/api/#{api_version}/changeset/1/upload", :method => :post },
          { :controller => "api/changesets", :action => "upload", :id => "1", :api_version => api_version }
        )
        assert_routing(
          { :path => "/api/#{api_version}/changeset/1/download", :method => :get },
          { :controller => "api/changesets", :action => "download", :id => "1", :api_version => api_version }
        )
        assert_routing(
          { :path => "/api/#{api_version}/changeset/1/expand_bbox", :method => :post },
          { :controller => "api/changesets", :action => "expand_bbox", :id => "1", :api_version => api_version }
        )
        assert_routing(
          { :path => "/api/#{api_version}/changeset/1", :method => :get },
          { :controller => "api/changesets", :action => "show", :id => "1", :api_version => api_version }
        )
        assert_routing(
          { :path => "/api/#{api_version}/changeset/1/subscribe", :method => :post },
          { :controller => "api/changesets", :action => "subscribe", :id => "1", :api_version => api_version }
        )
        assert_routing(
          { :path => "/api/#{api_version}/changeset/1/unsubscribe", :method => :post },
          { :controller => "api/changesets", :action => "unsubscribe", :id => "1", :api_version => api_version }
        )
        assert_routing(
          { :path => "/api/#{api_version}/changeset/1", :method => :put },
          { :controller => "api/changesets", :action => "update", :id => "1", :api_version => api_version }
        )
        assert_routing(
          { :path => "/api/#{api_version}/changeset/1/close", :method => :put },
          { :controller => "api/changesets", :action => "close", :id => "1", :api_version => api_version }
        )
        assert_routing(
          { :path => "/api/#{api_version}/changesets", :method => :get },
          { :controller => "api/changesets", :action => "query", :api_version => api_version }
        )
      end
    end
    ...
    def test_create_invalid_no_content
      all_api_versions.each do |api_version|
        ...
        basic_authorization nil, nil
        put :create, :params => { :api_version => api_version }
    ...
    def test_create_wrong_method
      all_api_versions.each do |api_version|
        basic_authorization create(:user).email, "test"
        get :create, :params => { :api_version => api_version }
        assert_response :method_not_allowed
        post :create, :params => { :api_version => api_version }
        assert_response :method_not_allowed
      end
    end
    ...
    def test_show
      all_api_versions.each do |api_version|
        ...
        get :show, :params => { :id => changeset.id, :api_version => api_version }
        ...
        get :show, :params => { :id => changeset.id, :include_discussion => true, :api_version => api_version }
    ...
    def test_show_not_found
      all_api_versions.each do |api_version|
        [0, -32, 233455644, "afg", "213"].each do |id|
          get :show, :params => { :id => id, :api_version => api_version }
    ...
    def test_close
      all_api_versions.each do |api_version|
        ...
        basic_authorization nil, nil
        put :close, :params => { :id => changeset.id, :api_version => api_version }
        assert_response :unauthorized

        ## Try using the non-public user
        basic_authorization private_user.email, "test"
        put :close, :params => { :id => private_changeset.id, :api_version => api_version }
        assert_require_public_data
        ...
        cs_id = changeset.id
        put :close, :params => { :id => cs_id, :api_version => api_version }
    ...
    def test_close_method_invalid
      all_api_versions.each do |api_version|
        ...
        get :close, :params => { :id => changeset.id, :api_version => api_version }
        assert_response :method_not_allowed

        post :close, :params => { :id => changeset.id, :api_version => api_version }
        assert_response :method_not_allowed
      end
    end
    ...
    def test_close_not_found
      all_api_versions.each do |api_version|
        ...
        basic_authorization nil, nil
        cs_ids.each do |id|
          put :close, :params => { :id => id, :api_version => api_version }
...
CHANGESET
        ...
        # upload it
        post :upload, :params => { :id => changeset_id, :api_version => api_version }, :body => diff
        assert_response :unauthorized,
...
CHANGESET
        ...
        # upload it
        post :upload, :params => { :id => changeset.id, :api_version => api_version }, :body => diff
        assert_response :success,
    ...
    def test_upload_delete
      all_api_versions.each do |api_version|
        ...
        # upload it
        post :upload, :params => { :id => changeset.id, :api_version => api_version }, :body => diff.to_s
        assert_response :success,
      ...
      all_api_versions.each do |api_version|
        3.times do
          ...
          assert_difference "Changeset.count", 1 do
            put :create, :params => { :api_version => api_version }, :body => xml
          end
          assert_response :success
        end
      end
    ...
    def test_upload_large_changeset
      all_api_versions.each do |api_version|
        ...
        # create a changeset
        put :create, :params => { :api_version => api_version }, :body => "<osm><changeset/></osm>"
...
CHANGESET
        ...
        # check the returned payload
        assert_select "diffResult[version='#{api_version}'][generator='#{Settings.generator}']", 1
        assert_select "diffResult>node", 1
        assert_select "diffResult>way", 1
        assert_select "diffResult>relation", 1
...
CHANGESET
    ...
    def test_upload_node_move
      all_api_versions.each do |api_version|
        ...
        # upload it
        post :upload, :params => { :id => changeset_id, :api_version => api_version }, :body => diff.to_s
        assert_response :success,
    ...
    def test_upload_xml_errors
      all_api_versions.each do |api_version|
        ...
        # check the returned payload
        assert_select "osmError[version='0.6'][generator='OpenStreetMap server']", 1
...
CHANGESET
        ...
        get :download, :params => { :id => changeset_id, :api_version => api_version }
        assert_response :success
...
CHANGESET
    ...
    def test_changeset_download
      all_api_versions.each do |api_version|
        ...
        get :download, :params => { :id => changeset.id, :api_version => api_version }

        assert_response :success
        assert_template nil
        # print @response.body
        # FIXME: needs more assert_select tests
        assert_select "osmChange[version='#{api_version}'][generator='#{Settings.generator}']" do
    ...
    def test_changeset_bbox
      all_api_versions.each do |api_version|
        ...
        with_controller(WaysController.new) do
          ...
          put :delete, :params => { :id => way.id, :api_version => api_version }, :body => xml.to_s
    ...
    def test_changeset_include
      all_api_versions.each do |api_version|
        ...
        check_after_include(api_version, changeset_id, 1, 1, [1, 1, 1, 1])
        check_after_include(api_version, changeset_id, 3, 3, [1, 1, 3, 3])
        check_after_include(api_version, changeset_id, 4, 2, [1, 1, 4, 3])
        check_after_include(api_version, changeset_id, 2, 2, [1, 1, 4, 3])
        check_after_include(api_version, changeset_id, -1, -1, [-1, -1, 4, 3])
        check_after_include(api_version, changeset_id, -2, 5, [-2, -1, 4, 5])
      end
    end
    ...
    def test_changeset_expand_bbox_error
      all_api_versions.each do |api_version|
        ...
        put :expand_bbox, :params => { :id => changeset_id, :api_version => api_version }, :body => xml
        assert_response :method_not_allowed, "shouldn't be able to put a bbox expand"

        # Try to get the update
        xml = "<osm><node lon='#{lon}' lat='#{lat}'/></osm>"
        get :expand_bbox, :params => { :id => changeset_id, :api_version => api_version }, :body => xml
        assert_response :method_not_allowed, "shouldn't be able to get a bbox expand"

        # Try to use a hopefully missing changeset
        xml = "<osm><node lon='#{lon}' lat='#{lat}'/></osm>"
        post :expand_bbox, :params => { :id => changeset_id + 13245, :api_version => api_version }, :body => xml
    ...
    def test_query
      ...
      all_api_versions.each do |api_version|
        get :query, :params => { :bbox => "-10,-10, 10, 10", :api_version => api_version }
        assert_response :success, "can't get changesets in bbox"
        assert_changesets [changeset2, changeset3]

        get :query, :params => { :bbox => "4.5,4.5,4.6,4.6", :api_version => api_version }
        assert_response :success, "can't get changesets in bbox"
        assert_changesets [changeset3]

        # not found when looking for changesets of non-existing users
        get :query, :params => { :user => User.maximum(:id) + 1, :api_version => api_version }
        assert_response :not_found
        get :query, :params => { :display_name => " ", :api_version => api_version }
        assert_response :not_found

        # can't get changesets of user 1 without authenticating
        basic_authorization nil, nil
        get :query, :params => { :user => private_user.id, :api_version => api_version }
        assert_response :not_found, "shouldn't be able to get changesets by non-public user (ID)"
        get :query, :params => { :display_name => private_user.display_name, :api_version => api_version }
        ...
        get :query, :params => { :user => private_user.id, :open => true, :api_version => api_version }
        assert_response :success, "can't get changesets by user and open"
        assert_changesets [private_user_changeset]

        get :query, :params => { :time => "2007-12-31", :api_version => api_version }
        ...
        get :query, :params => { :time => "2008-01-01T12:34Z", :api_version => api_version }
        assert_response :success, "can't get changesets by time-since with hour"
        assert_changesets [private_user_changeset, private_user_closed_changeset, changeset, closed_changeset, changeset2, changeset3]

        get :query, :params => { :time => "2007-12-31T23:59Z,2008-01-02T00:01Z", :api_version => api_version }
        assert_response :success, "can't get changesets by time-range"
        assert_changesets [closed_changeset]

        get :query, :params => { :open => "true", :api_version => api_version }
        assert_response :success, "can't get changesets by open-ness"
        assert_changesets [private_user_changeset, changeset, changeset2, changeset3]

        get :query, :params => { :closed => "true", :api_version => api_version }
        assert_response :success, "can't get changesets by closed-ness"
        assert_changesets [private_user_closed_changeset, closed_changeset]

        get :query, :params => { :closed => "true", :user => private_user.id, :api_version => api_version }
        assert_response :success, "can't get changesets by closed-ness and user"
        assert_changesets [private_user_closed_changeset]

        get :query, :params => { :closed => "true", :user => user.id, :api_version => api_version }
        assert_response :success, "can't get changesets by closed-ness and user"
        assert_changesets [closed_changeset]

        get :query, :params => { :changesets => "#{private_user_changeset.id},#{changeset.id},#{closed_changeset.id}", :api_version => api_version }
        ...
        get :query, :params => { :changesets => "", :api_version => api_version }
      ...
      all_api_versions.each do |api_version|
        ["abracadabra!",
         ...
         ";drop table users;"].each do |bbox|
          get :query, :params => { :bbox => bbox, :api_version => api_version }
        ...
        ["now()",
         ...
         ",",
         "-,-"].each do |time|
          get :query, :params => { :time => time, :api_version => api_version }
        ...
        ["me",
         ...
         "-1",
         "0"].each do |uid|
          get :query, :params => { :user => uid, :api_version => api_version }
    ...
    def test_changeset_update
      all_api_versions.each do |api_version|
        ...
        basic_authorization nil, nil
        put :update, :params => { :id => private_changeset.id, :api_version => api_version }, :body => new_changeset.to_s
        assert_response :unauthorized
        ...
        put :update, :params => { :id => changeset.id, :api_version => api_version }, :body => new_changeset.to_s
        assert_response :unauthorized
    ...
    def test_changeset_limits
      all_api_versions.each do |api_version|
        ...
        with_controller(NodesController.new) do
          ...
          get :show, :params => { :id => node_id, :api_version => api_version }
          ...
          offset.times do |i|
            ...
            put :update, :params => { :id => node_id, :api_version => api_version }, :body => node_doc.to_s
    ...
    def test_subscribe_success
      all_api_versions.each do |api_version|
        ...
        assert_difference "changeset.subscribers.count", 1 do
          post :subscribe, :params => { :id => changeset.id, :api_version => api_version }
        end
        assert_response :success
    ...
    def test_subscribe_fail
      all_api_versions.each do |api_version|
        ...
        # bad changeset id
        assert_no_difference "changeset.subscribers.count" do
          post :subscribe, :params => { :id => 999111, :api_version => api_version }
        end
        assert_response :not_found
    ...
    def test_unsubscribe_success
      all_api_versions.each do |api_version|
        ...
        assert_difference "changeset.subscribers.count", -1 do
          post :unsubscribe, :params => { :id => changeset.id, :api_version => api_version }
        end
        assert_response :success
    ...
    def test_unsubscribe_fail
      all_api_versions.each do |api_version|
        ...
        # bad changeset id
        assert_no_difference "changeset.subscribers.count" do
          post :unsubscribe, :params => { :id => 999111, :api_version => api_version }
        end
        assert_response :not_found
    ...
    def check_after_include(api_version, changeset_id, lon, lat, bbox)
      xml = "<osm><node lon='#{lon}' lat='#{lat}'/></osm>"
      post :expand_bbox, :params => { :id => changeset_id, :api_version => api_version }, :body => xml
