...
class FollowsControllerTest < ActionDispatch::IntegrationTest
  ...
  def test_routes
    assert_routing(
      { :path => "/user/username/follow/edit", :method => :get },
      { :controller => "follows", :action => "edit", :display_name => "username" }
    )
    assert_routing(
  ...
  def test_follow
    ...
    assert_nil Follow.find_by(:befriender => user, :befriendee => follow)

    # When not logged in a GET should ask us to login
    get edit_follow_path(follow)
    assert_redirected_to login_path(:referer => edit_follow_path(follow))
    ...
    assert Follow.find_by(:befriender => user, :befriendee => follow)
  ...
  def test_follow_with_referer
    ...
    get edit_follow_path(follow), :params => { :referer => "/test" }
    assert_response :success
    assert_template :edit
  ...
  def test_follow_unknown_user
    ...
    get edit_follow_path("No Such User")
  ...
  def test_unfollow
    ...
    user = create(:user)
    follow = create(:user)
    create(:follow, :befriender => user, :befriendee => follow)
    ...
    # When logged in a DELETE should remove the friendship
    delete follow_path(follow)
    assert_redirected_to user_path(follow)
    assert_match(/You successfully unfollowed/, flash[:notice])
    assert_nil Follow.find_by(:befriender => user, :befriendee => follow)

    # A second DELETE should report that the friendship does not exist
    delete follow_path(follow)
    assert_redirected_to user_path(follow)
