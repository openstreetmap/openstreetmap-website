...
class UserActivitiesTest < ActiveSupport::TestCase
  ...
  def test_for_user_returns_activities_grouped_by_day
    ...
    first_day_count = first_day["activity_groups"].values.sum { |g| g["count"] }
    second_day_count = second_day["activity_groups"].values.sum { |g| g["count"] }
  ...
  def test_activities_include_all_types
    ...
    activities.each do |day|
      day["activity_groups"].each_value do |group|
        items = JSON.parse(group["items"])
