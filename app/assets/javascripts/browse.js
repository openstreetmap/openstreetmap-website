function startBrowse(sidebarHtml) {
  ...
  $("#sidebar_title").html(i18n('browse.start_rjs.data_frame_title'));
  ...
  $("#browse_hide_areas_box").html(i18n('browse.start_rjs.hide_areas'));
  ...
  function updateData() {
    if (browseMode == "auto") {
      if (map.getZoom() >= 15) {
          ...
          setStatus(i18n('browse.start_rjs.zoom_or_select'));
      }
    }
  }
  ...
  function startDrag() {
    $("#browse_select_box").html(i18n('browse.start_rjs.drag_a_box'));
  ...
  function hideAreas() {
    $("#browse_hide_areas_box").html(i18n('browse.start_rjs.show_areas'));
    $("#browse_hide_areas_box").show();
  ...
  function endDrag(bbox) {
    ...
    $("#browse_select_box").html(i18n('browse.start_rjs.manually_select'));
  ...
  function displayFeatureWarning(count, limit, callback) {
    ...
    p.appendChild(document.createTextNode(i18n('browse.start_rjs.loaded_an_area_with_num_features', { num_features: count, max_features: limit })));
    div.appendChild(p);

    var input = document.createElement("input");
    input.type = "submit";
    input.value = i18n('browse.start_rjs.load_data');
  ...
  function getData(bounds, reload) {
    ...
    if (size > OSM.MAX_REQUEST_AREA) {
      setStatus(i18n('browse.start_rjs.unable_to_load_size', { max_bbox_size: OSM.MAX_REQUEST_AREA, bbox_size: size }));
    } else {
  ...
  function loadData(url, reload) {
    setStatus(i18n('browse.start_rjs.loading'));
  ...
  function dataLoaded() {
    if (this.map.dataLayer.active) {
      ...
      heading.appendChild(document.createTextNode(i18n('browse.start_rjs.object_list.heading')));
      ...
      link.appendChild(document.createTextNode(i18n('browse.start_rjs.object_list.api')));
  ...
  function onFeatureSelect(feature) {
    ...
    link.appendChild(document.createTextNode(i18n('browse.start_rjs.object_list.back')));
    ...
    var link = document.createElement("a");
    link.href = "/browse/" + type + "/" + feature.osm_id;
    link.appendChild(document.createTextNode(i18n('browse.start_rjs.object_list.details')));
    td.appendChild(link);
    ...
    link.appendChild(document.createTextNode(i18n('browse.start_rjs.show_history')));
  ...
  function loadHistory() {
    this.link.href = "";
    this.link.innerHTML = i18n('browse.start_rjs.wait');
  ...
  function displayHistory(request) {
    ...
    var heading = document.createElement("td");
    heading.appendChild(document.createTextNode(i18n('browse.start_rjs.history_for_feature', { feature: featureNameHistory(this.feature) })));
    tr.appendChild(heading);
    ...
    link.appendChild(document.createTextNode(i18n('browse.start_rjs.details')));
    td.appendChild(link);
    ...
    for (var i = nodes.length - 1; i >= 0; i--) {
      var user = nodes[i].getAttribute("user") || i18n('browse.start_rjs.private_user');
      var timestamp = nodes[i].getAttribute("timestamp");
      var item = document.createElement("li");
      item.appendChild(document.createTextNode(i18n('browse.start_rjs.edited_by_user_at_timestamp', { user: user, timestamp: timestamp })));
  ...
  function featureTypeName(feature) {
    if (featureType(feature) == "node") {
      return i18n('browse.start_rjs.object_list.type.node');
    } else if (featureType(feature) == "way") {
      return i18n('browse.start_rjs.object_list.type.way');
    }
  }
  ...
  function featureNameSelect(feature) {
    ...
    if (feature.attributes['name:' + lang]) {
      ...
      return i18n('browse.start_rjs.object_list.selected.type.node', { id: feature.osm_id });
    } else if (featureType(feature) == "way") {
      return i18n('browse.start_rjs.object_list.selected.type.way', { id: feature.osm_id });
    }
  }
  ...
  function featureNameHistory(feature) {
    ...
    if (feature.attributes['name:' + lang]) {
      ...
      return i18n('browse.start_rjs.object_list.history.type.node', { id: feature.osm_id });
    } else if (featureType(feature) == "way") {
      return i18n('browse.start_rjs.object_list.history.type.way', { id: feature.osm_id });
    }
  }
