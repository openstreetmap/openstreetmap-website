    ...
    feed.entry(changeset, :updated => changeset.closed_at, :id => changeset_url(changeset.id, :only_path => false)) do |entry|
      ...
      if changeset.user.data_public?
        entry.author do |author|
          ...
          author.uri url_for(:controller => "user", :action => "show", :display_name => changeset.user.display_name, :only_path => false)
        end
      end
        ...
        xhtml.table do |table|
          ...
          if changeset.user.data_public?
            table.tr do |tr|
              ...
              tr.td do |td|
                td.a h(changeset.user.display_name), :href => url_for(:controller => "user", :action => "show", :display_name => changeset.user.display_name, :only_path => false)
              end
            end
          end
