...
class AmfController < ApplicationController
  ...
  def amf_read
    ...
    self.response_body = Dispatcher.new(request.raw_post) do |message, *args|
      ...
      result = case message
                ...
                when 'whichways'
                  whichways(*args)
                when 'whichways_deleted'
                  whichways_deleted(*args)
                when 'getway'
                  getway(args[0].to_i)
                when 'getrelation'
                  getrelation(args[0].to_i)
                when 'getway_old'
                  getway_old(args[0].to_i,args[1])
                when 'getway_history'
                  getway_history(args[0].to_i)
                when 'getnode_history'
                  getnode_history(args[0].to_i)
                when 'findgpx'
                  findgpx(*args)
                when 'findrelations'
                  findrelations(*args)
    ...
    self.response_body = Dispatcher.new(request.raw_post) do |message, *args|
      ...
      if err
        ...
        when 'putway'
          ...
          if result[0] == 0 && result[2] != result[3]
            renumberedways[result[2]] = result[3]
          end
        when 'putrelation'
        ...
        when 'putpoi'
          result = putpoi(*args)
          if result[0] == 0 && result[2] != result[3]
            renumberednodes[result[2]] = result[3]
          end
        when 'startchangeset'
  ...
  def startchangeset(usertoken, cstags, closeid, closecomment, opennew)
    amf_handle_error("'startchangeset'", nil, nil) do
      ...
      if !user
        return -1, "You are not logged in, so Potlatch can't write any changes to the database."
      end
      if user.blocks.active.exists?
        return -1, t('application.setup_user_auth.blocked')
      end
      # TODO: refactor into method?
      if REQUIRE_TERMS_AGREED && user.terms_agreed.nil?
        return -1, "You must accept the contributor terms before you can edit."
      end

      if cstags
        if !tags_ok(cstags)
          return -1, "One of the tags is invalid. Linux users may need to upgrade to Flash Player 10.1."
        end
      ...
      if closeid
        ...
        raise OSM::APIUserChangesetMismatchError.new if cs.user_id != user.id
  ...
  def whichways(xmin, ymin, xmax, ymax) #:doc:
    amf_handle_error_with_timeout("'whichways'", nil, nil) do
      ...
      if POTLATCH_USE_SQL then
        ...
        relations = sql_find_relations_in_area_and_ways(bbox, ways.map { |x| x[0] })
      else
        ...
        ways = nodes_in_area.inject([]) do |sum, node| 
          ...
          sum + visible_ways.map { |w| [w.id, w.version] }
        end.uniq
        ways.delete([])

        # find the node ids in an area that aren't part of ways
        nodes_not_used_in_area = nodes_in_area.select { |node| node.ways.empty? }
        points = nodes_not_used_in_area.map { |n| [n.id, n.lon, n.lat, n.tags, n.version] }.uniq

        # find the relations used by those nodes and ways
        relations = Relation.nodes(nodes_in_area.map { |n| n.id }).visible +
                    Relation.ways(ways.map { |w| w[0] }).visible
        relations = relations.map { |relation| [relation.id,relation.version] }.uniq
      end
  ...
  def whichways_deleted(xmin, ymin, xmax, ymax) #:doc:
    amf_handle_error_with_timeout("'whichways_deleted'", nil, nil) do
      ...
      way_ids = nodes_in_area.map { |node| node.ways_via_history.invisible.map { |way| way.id } }.flatten.uniq
  ...
  def getway_history(wayid) #:doc:
    begin
      ...
      revdates=[]
      revusers={}
      Way.find(wayid).old_ways.unredacted.map do |a|
        revdates.push(a.timestamp)
        unless revusers.has_key?(a.timestamp.to_i)
          revusers[a.timestamp.to_i]=change_user(a)
        end
        a.nds.each do |n|
          Node.find(n).old_nodes.unredacted.map do |o|
            revdates.push(o.timestamp)
            unless revusers.has_key?(o.timestamp.to_i)
              revusers[o.timestamp.to_i]=change_user(o)
            end
          end
        end
      end
      ...
      # Map all in one nested array
      revdates.map! { |d| [(d + 1).strftime("%d %b %Y, %H:%M:%S")] + revusers[d.to_i] }
      revdates.uniq!
  ...
  def getnode_history(nodeid) #:doc:
    begin 
      history = Node.find(nodeid).old_nodes.unredacted.reverse.map do |old_node|
  ...
  def findgpx(searchterm, usertoken)
    amf_handle_error_with_timeout("'findgpx'", nil, nil) do
      user = getuser(usertoken)
      return [-1, "You must be logged in to search for GPX traces."] if !user
  ...
  def putrelation(renumberednodes, renumberedways, usertoken, changeset_id, version, relid, tags, members, visible) #:doc:
    amf_handle_error("'putrelation' #{relid}", 'relation', relid)  do
      user = getuser(usertoken)
      return [-1, "You are not logged in, so the relation could not be saved."] if !user
      return [-1, t('application.setup_user_auth.blocked')] if user.blocks.active.exists?
      # TODO: refactor into method?
      if REQUIRE_TERMS_AGREED && user.terms_agreed.nil?
        return [-1, "You must accept the contributor terms before you can edit."]
  ...
  def putway(renumberednodes, usertoken, changeset_id, wayversion, originalway, pointlist, attributes, nodes, deletednodes) #:doc:
    amf_handle_error("'putway' #{originalway}", 'way', originalway) do
      ...
      user = getuser(usertoken)
      return [-1,"You are not logged in, so the way could not be saved."] if !user
      return [-1, t('application.setup_user_auth.blocked')] if user.blocks.active.exists?
      # TODO: refactor into method?
      ...
      return [-1, "One of the tags is invalid. Linux users may need to upgrade to Flash Player 10.1."] if !tags_ok(attributes)

      attributes = strip_non_xml_chars attributes

      originalway = originalway.to_i
      pointlist.map! { |a| a.to_i }
  ...
  def putpoi(usertoken, changeset_id, version, id, lon, lat, tags, visible) #:doc:
    amf_handle_error("'putpoi' #{id}", 'node',id) do
      user = getuser(usertoken)
      return [-1,"You are not logged in, so the point could not be saved."] if !user
      return [-1, t('application.setup_user_auth.blocked')] if user.blocks.active.exists?
      # TODO: refactor into method?
      if REQUIRE_TERMS_AGREED && user.terms_agreed.nil?
        return [-1, "You must accept the contributor terms before you can edit."]
  ...
  def getpoi(id, timestamp) #:doc:
    amf_handle_error("'getpoi' #{id}" ,'node', id) do
      ...
      v = n.version
      unless timestamp == ''
        n = OldNode.where("node_id = ? AND timestamp <= ?", id, timestamp).
                      unredacted.
  ...
  def deleteway(usertoken, changeset_id, way_id, way_version, deletednodes) #:doc:
    amf_handle_error("'deleteway' #{way_id}", 'way', way_id) do
      user = getuser(usertoken)
      return [-1, "You are not logged in, so the way could not be deleted."] if !user
      return [-1, t('application.setup_user_auth.blocked')] if user.blocks.active.exists?
      # TODO: refactor into method?
      if REQUIRE_TERMS_AGREED && user.terms_agreed.nil?
        return -1,"You must accept the contributor terms before you can edit."
  ...
  def getlocales
    Dir.glob("#{Rails.root}/config/potlatch/locales/*").map { |f| File.basename(f, ".yml") }
  end
  
  ##
  # check that all key-value pairs are valid UTF-8.
  def tags_ok(tags)
    # TODO use Enumerable#all?
    tags.each do |k, v|
  ...
  def sql_find_ways_in_area(bbox)
    ...
    EOF
    ActiveRecord::Base.connection.select_all(sql).map { |a| [a['wayid'].to_i,a['version'].to_i] }
  end
  ...
  def sql_find_relations_in_area_and_ways(bbox, way_ids)
    ...
    ActiveRecord::Base.connection.select_all(sql).map { |a| [a['relid'].to_i,a['version'].to_i] }
  end
